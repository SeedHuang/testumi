import { TreeNode, createResource } from '@dn/core';
import {
    DnFC,
    DroppableWidget,
    TreeNodeWidget,
            ...
            <LoadTemplate
                actions={[
...

                    {
                        ...
                        icon: 'AddOperation',
                        onClick: () => {
                            const oldAdditionNode = findNodeByComponentPath(
                                node,
                                ['ArrayCards', 'ArrayCards.Addition'],
                            );
                            if (!oldAdditionNode) {
                                const additionNode = new TreeNode({
                                    componentName: node.componentName,
                                ...
                                ensureObjectItemsNode(node).insertAfter(
                                    additionNode,
                                );
                            }
                            const oldRemoveNode = findNodeByComponentPath(
                                node,
                                ['ArrayCards', '*', 'ArrayCards.Remove'],
                            );
                            const oldMoveDownNode = findNodeByComponentPath(
                                node,
                                ['ArrayCards', '*', 'ArrayCards.MoveDown'],
                            );
                            const oldMoveUpNode = findNodeByComponentPath(
                                node,
                                ['ArrayCards', '*', 'ArrayCards.MoveUp'],
                            );
                            if (!oldRemoveNode) {
